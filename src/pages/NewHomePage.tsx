...
const NewHomePage = () => {
  return (
    <div className="min-h-screen bg-brand-darkest">
      ...
      {/* Features Section */}
      <section className="container mx-auto px-6 py-16 grid grid-cols-3 gap-8">
        {[
          {
            title: "Free",
          ...
          <motion.div 
            key={feature.title}
            className="p-6 rounded-3xl border-2 border-brand-light bg-brand-dark/30 text-center"
      ...
      <section className="bg-brand-dark py-16">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-4 gap-8">
            {[1, 2, 3, 4].map((item) => (
              <div key={item} className="p-6 rounded-3xl border-2 border-brand-light bg-brand-darkest/50 text-center">
      ...
      {/* Footer */}
      <footer className="bg-brand-darkest py-8">
        <div className="container mx-auto px-6">
          <div className="flex justify-center space-x-8 mb-8">
            {['Contact', 'Send feedback', 'Request channel'].map((text) => (
              <button
                key={text}
                className="px-8 py-3 rounded-full border-2 border-brand-light text-white hover:bg-brand-light hover:text-brand-darkest transition-all duration-300"
              >
                {text}
              </button>
            ))}
          </div>
          <p className="text-center text-brand-light">All rights reserved @YidVid</p>
        </div>
      </footer>
    </div>
  );
