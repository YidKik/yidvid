...
export const VideoInfo = ({ 
  ...
  return (
    <div className="space-y-2 md:space-y-4">
      <h1 className="text-lg md:text-2xl font-bold mb-1 md:mb-2">{title}</h1>
      <div className="flex items-center justify-between mb-2 md:mb-4">
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <p className="text-sm md:text-base text-muted-foreground">{channelName}</p>
        </div>
      ...
      {description && (
        <div className="mt-2 md:mt-4 p-2 md:p-4 bg-muted rounded-lg">
          <p className="whitespace-pre-wrap text-xs md:text-sm">{description}</p>
        </div>
      )}
    </div>
  );
