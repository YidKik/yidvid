...
export function AdminDashboardCards({ stats, notifications }: AdminDashboardCardsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate('/admin?tab=content-analysis')}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">AI Content Filtering</CardTitle>
          <Filter className="h-4 w-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{stats?.aiPending ?? stats?.pendingAnalysis ?? 0}</div>
          <p className="text-xs text-muted-foreground">
            Items pending analysis
          </p>
          <Button variant="outline" size="sm" className="mt-2">
            Open Filtering Dashboard
          </Button>
        </CardContent>
      </Card>
