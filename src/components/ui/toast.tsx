...
>(({ className, ...props }, ref) => {
  ...
  return (
    <ToastPrimitives.Viewport
      ref={ref}
      className={cn(
        "fixed z-[100] flex max-h-screen w-full flex-col-reverse p-2",
        isMobile 
          ? "bottom-0 items-center justify-center" 
          : "top-0 right-0 sm:bottom-0 sm:top-auto sm:flex-col md:max-w-[420px]",
        className
      )}
      {...props}
    />
  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-md border p-4 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80",
  {
    variants: {
      variant: {
        ...
        destructive:
          "border-destructive bg-white text-black",
      },
    },
    defaultVariants: {
      variant: "default",
  ...
  return (
    <ToastPrimitives.Root
      ...
      className={cn(
        ...
        className,
        isMobile ? "data-[state=open]:slide-in-from-bottom-full data-[state=closed]:slide-out-to-bottom-full" : 
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn("text-xs font-semibold text-black", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-[10px] opacity-90 text-gray-600", className)}
    {...props}
  />
))
