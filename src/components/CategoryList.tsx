...
export function CategoryList({ categories, onUpdate }: CategoryListProps) {
  ...
  const handleDeleteCategory = async (id: string) => {
    ...
    if (id.length === 36) { // UUID length check
      try {
        ...
        toast.success("Category deleted successfully", { id: `category-deleted-${id}` });
        onUpdate();
      } catch (error) {
        console.error("Error deleting category:", error);
        toast.error("Failed to delete category", { id: "category-delete-error" });
      }
    } else {
      toast.error("Default categories cannot be deleted", { id: "default-category-delete-error" });
    }
  };
  ...
  const handleSaveContent = async (selectedVideos: string[], selectedChannels: string[]) => {
    ...
    try {
      ...
      toast.success("Category content updated successfully", { id: `category-content-saved-${selectedCategory.id}` });
      setSelectedCategory(null);
    } catch (error) {
      console.error("Error updating category content:", error);
      toast.error("Failed to update category content", { id: "category-content-save-error" });
    }
  };
