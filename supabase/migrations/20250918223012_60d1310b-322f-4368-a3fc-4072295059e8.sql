-- Create YouTube channels table (if not exists)
...
-- Create YouTube videos table (if not exists)
...
DROP POLICY IF EXISTS "Public can view active channels" ON public.youtube_channels;
...
DROP POLICY IF EXISTS "Public can view approved videos" ON public.youtube_videos;
...
DROP POLICY IF EXISTS "Admins can manage all videos" ON public.youtube_videos;
